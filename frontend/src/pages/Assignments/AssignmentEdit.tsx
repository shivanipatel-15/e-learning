...
import { getAssignmentDetail, editAssignment }  from './../../api/assignmentAction'
...
const AssignmentEdit: React.FC<Props> = ({ rootStore }) => {
    const { assignment_id } = useParams<ParamTypes>()
    const [assignmentType, setAssignmentType] = useState('')
    const [assignment, setAssignmentDetail] = useState<any>()
    ...
    const onSubmit = async (data: any) => {
        ...
        try {
            const response = await editAssignment(postData)
            const responseData = response.data
    ...
    const getAssignments = async (assignment_id: string) => {
        try {
            ...
            setAssignmentType(responseData.data.assignment.assignment_type)
        } catch (error) {
            errorToast('Error! something is wrong')
        }
    }
            ...
            <div className="col-md-6 col-12 my-5">
                <div className="card">  
                    <div className="card-body">
                        <h5 className="card-title"><HistoryBack /> Edit Assignment </h5>
                        { 
                            assignment && 
                            <form onSubmit={handleSubmit(onSubmit)}>
                                            ...
                                            <div>
                                                <div className="form-check form-check-inline">
                                                    <input className="form-check-input"
                                                        ...
                                                        id="pdf"
                                                        value="pdf"
                                                        defaultChecked={assignment.assignment_type === 'pdf' ? true : false } 
                                                ...
                                                <div className="form-check form-check-inline">
                                                    <input
                                                        ...
                                                        id="google_form"
                                                        value="google_form"
                                                        defaultChecked={assignment.assignment_type === 'google_form' ? true : false } 
                                                        onClick={(e) => onAssignmentTypeChange('google_form')}
                                                        {...register('assignment_type', { required: true })} />
                                        ...
                                        }
                                        {
                                            (assignmentType === 'google_form') && <div className="form-group">
                                                ...
                                                <input
                                                    ...
                                                    defaultValue={assignment.attachment}
                                ...
                                <div className="form-group">
                                    ...
                                    <select
                                        ...
                                        defaultValue={`${assignment.standard}|${assignment.division}|${assignment.subject}`}
                                    >
                                        <option value=''>--- Select Standard and class ---</option>
                                        {subjects.map((subject: any) => {
                                            return <option 
                                                key={v4()} 
                                                value={`${subject.standard}|${subject.division}|${subject.subject}`}
                                            >{subject.subject} - {subject.division} ( {subject.subject} )</option>
                                        })}
                                    </select>
                                ...
                                <div className="form-group">
                                    ...
                                    <input 
                                        ...
                                        type="datetime-local" 
                                        id="submission_due" 
                                        defaultValue={moment(assignment.submission_due).format('YYYY-MM-DDTHH:mm')}
